...
export class ElementFormComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogref: MatDialogRef<ElementFormComponent>,
    ...
    this.serviceCategoria.getCategorias().subscribe(
      (data: any) => {
        ...
        tomaresultado.forEach((n, index) => {
          if (n.father_category_id != null) {
            let x = arregloaux.find(
              (element) => element.id_category == n.father_category_id
            );
            if (x != undefined) {
              let auxasig;
  ...
  enviar() {
    ...
    this.form.value.category_of_element_id.forEach((cat) => {
      if (cat) z.push({ category_of_element_id: cat.id_category });
    });
          ...
          this.marcaService.getMarcas().subscribe(
            ...
            (err) => {
              ...
              window.location.assign(
                'https://sedacreditaciones.com/app/patrimonio'
              );
            }
          );
        }
  ...
  select(cat) {
    ...
    if (
      !arrayaux.find((element: any) => element.id_category == cat.id_category)
    )
